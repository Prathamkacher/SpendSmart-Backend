...
class JwtAuthenticationFilterTest {
    ...
    void doFilterInternal_ShouldPassThroughWithoutBearerHeader() throws Exception {
        JwtAuthenticationFilter filter = new JwtAuthenticationFilter(jwtService, new ObjectMapper().findAndRegisterModules());
    ...
    void doFilterInternal_ShouldPopulateAuthenticationForValidToken() throws Exception {
        ...
        when(jwtService.extractEmail("valid-token")).thenReturn("notify@example.com");
        when(jwtService.extractUserId("valid-token")).thenReturn(12L);
        when(jwtService.isTokenValid("valid-token", "notify@example.com")).thenReturn(true);
