...
class JwtAuthenticationFilterTest {
    ...
    @Test
    void doFilterInternal_ShouldAuthenticateValidTokenAndUseDefaultRoleWhenMissing() throws Exception {
        JwtAuthenticationFilter filter = new JwtAuthenticationFilter(jwtService, objectMapper());
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addHeader("Authorization", "Bearer valid-token");
        MockHttpServletResponse response = new MockHttpServletResponse();

        when(jwtService.extractEmail("valid-token")).thenReturn("category@example.com");
        when(jwtService.isTokenValid("valid-token", "category@example.com")).thenReturn(true);
        when(jwtService.extractUserId("valid-token")).thenReturn(22L);
        when(jwtService.extractRole("valid-token")).thenReturn(null);

        filter.doFilter(request, response, filterChain);

        verify(filterChain).doFilter(request, response);
        assertThat(request.getAttribute("userId")).isEqualTo(22L);
